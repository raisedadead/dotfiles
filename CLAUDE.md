...
- **OMP never reaps its per-session cache files** — `.zshrc` keeps the newest 50 `shell.*.omp.cache`/`zsh.*.omp.cache` in a disowned background block. The slice **must** be `"${(@)_c[51,-1]}"`; a plain `"${_c[51,-1]}"` joins the whole slice into one word and silently deletes nothing (this shipped broken and grew 8.5k files)
