...
SessionStart registers the main marker and sets the session title. The doctor runs beside it as an `asyncRewake` hook at startup and wakes Claude with the issue lines when it finds any. In `--rewake` mode it exits 0 at once unless the session's `cwd` is in the `PUB_SOURCE` repository, one of its worktrees, or one of its submodules. It reads that `cwd` from the hook payload on stdin and falls back to `$PWD`. A different repository gets no check and no wake, because you can only repair the drift in the source repository. TaskCreated denies a task name outside the `<GROUP><N> <title>` form. PreToolUse evaluates command and file rules and spawn contracts. PostToolUse formats, validates, queues project checks, and records mutations. SubagentStop clears the mutation ledger when a review agent completes. Stop handles failure suppression, review, claims, queued validators, and notification; a continued Stop skips the gates and the sound but still drains the validator queue. SessionEnd cleans session state. Read the dispatcher for the other events and the error paths.
