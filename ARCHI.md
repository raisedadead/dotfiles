...
Edit target → validate → `chezmoi status` → `chezmoi re-add <target>` → inspect Git diff → commit. The source is `~/.dotfiles`. Nothing captures a target on its own: a target edit stays visible in `chezmoi status` and `chezmoi diff` until you capture it with `re-add` or overwrite it with `apply`. Editor saves do not apply source files automatically; `chezmoi edit --apply <target>` is the explicit source-edit path. Run `chezmoi add <target>` to manage a new file. Settings: [.chezmoi.toml.tmpl](.chezmoi.toml.tmpl).

Read `chezmoi status` before a bare `chezmoi re-add`. A bare `re-add` captures every modified target, including an installer or runtime write you did not make. When the list shows a change you did not make, capture one target at a time and revert the rest with `chezmoi apply <target>`.

`dot_claude/` is a git submodule of the private repo `raisedadead/dotfiles-private`, branch `dot_claude`; a capture of a `~/.claude` target lands there. Commit inside `dot_claude/`. Its `.githooks/post-commit` then commits the gitlink bump in `~/.dotfiles` as `chore(dot_claude): bump to <sha>`; it exits 0 without a commit when there is no superproject, when the parent is mid-merge or mid-rebase, or when `HEAD` already records the gitlink. When the parent tip is a bump that no remote holds, the hook amends it, so consecutive submodule commits before a push leave one bump commit. The bump takes the parent author and date. `status.submoduleSummary` lists any submodule commit the parent does not record yet. `dotfiles-privatize.sh` seeds `pre-commit` and `post-commit` from the parent `.githooks/` into each new private branch. Both repos run `gitleaks` in `.githooks/pre-commit` and exit 1 on a finding. `.githooks/pre-push` in the parent resolves the gitlink of each commit in each pushed range and exits 1 when that submodule commit is on no remote, or when it cannot check it, because a clone of the parent cannot then check that submodule out. A machine that ran the old `install.sh` still has the retired capture agent loaded; unload it with `launchctl bootout gui/$UID/dev.mrugesh.chezmoi-autocapture` and delete its plist under `~/Library/LaunchAgents`. `submodule.recurse` is `false`, because `true` makes `pull`, `checkout`, `switch` and `reset` overwrite the submodule working tree with the recorded gitlink and rewind newer work. `submodule.dot_claude.update` is `merge`, so an explicit `git submodule update` is a no-op while the branch is ahead. `push.recurseSubmodules` is `on-demand`. Each private directory is one orphan branch named after it, mounted with `git submodule add -b <dir>`. A private branch root may hold only `.git*`-prefixed files; chezmoi skips those and deploys anything else under the target.
