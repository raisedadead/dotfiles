...
Layouts live in ~/.config/herdr/layouts/<name>.toml, or as .herdr/layout.toml
inside a repository.
...
def discover() -> dict[str, str]:
    found = {}
    local = os.path.join(os.getcwd(), ".herdr", "layout.toml")
...
def build(spec: dict, path: str) -> str:
    ...
    for index, tab in enumerate(tabs):
        ...
        for position, command in enumerate(commands):
            if position > 0:
                direction = "right" if pane_width(pane_id) >= WIDE_COLUMNS else "down"
                pane_id = herdr(
                    "pane",
                    "split",
                    "--pane",
            ...
            if str(command).strip():
                herdr("pane", "run", pane_id, str(command))
...
def main() -> int:
    ...
    except (OSError, tomllib.TOMLDecodeError, RuntimeError, KeyError) as err:
