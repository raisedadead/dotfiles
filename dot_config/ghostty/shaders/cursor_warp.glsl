...
// credits: https://github.com/sahaj-b
...
const float FADE_EXPONENT = 0.8;

const float TRAIL_RAINBOW = 1.0;
const float TRAIL_RAINBOW_SPEED = 0.9;
const float TRAIL_RAINBOW_SPREAD = 0.8;
const float TRAIL_RAINBOW_SATURATION = 0.7;
const float TRAIL_RAINBOW_VALUE = 1.0;
const float TRAIL_RAINBOW_OPACITY = 1.0;

float ease(float x) {
    return 1.0 - (1.0 - x) * (1.0 - x);
}

vec3 trailHueToRgb(float h, float s, float v) {
...
void mainImage(out vec4 fragColor, in vec2 fragCoord){
    ...
    if (lineLength > minDist && baseProgress < DURATION - 0.001) {
        ...
        float trailHue = fract(iTime * TRAIL_RAINBOW_SPEED + fadeProgress * TRAIL_RAINBOW_SPREAD);
        vec3 trailRainbow = trailHueToRgb(trailHue, TRAIL_RAINBOW_SATURATION, TRAIL_RAINBOW_VALUE);
        trail.rgb = mix(trail.rgb, trailRainbow, TRAIL_RAINBOW);
        trail.a = mix(trail.a, TRAIL_RAINBOW_OPACITY, TRAIL_RAINBOW);
