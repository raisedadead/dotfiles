...
Managed with [`chezmoi`](https://www.chezmoi.io/). Previously managed with [`homeshick`](https://github.com/andsens/homeshick), which served reliably for years.
...
Homebrew package management lives in a separate repo by design. When that repo is applied, it provides `~/.config/brewfile/Brewfile`.
...
1. Set up [1Password](https://1password.com/downloads/mac/) and enable the SSH agent under **Settings → Developer → SSH Agent**.

1. Apply public dotfiles:

   ```bash
   chezmoi init git@github.com:raisedadead/dotfiles.git --source ~/.dotfiles --apply
   ```

1. Apply private dotfiles:
...
1. Install packages (optional, if your separate Brewfile repo is already applied):
...
`home` is the day-to-day entrypoint. It wraps the public + private chezmoi repos and gives you status, sync, apply, pull, and push commands that work across both.
